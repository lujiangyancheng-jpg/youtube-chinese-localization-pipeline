...
下载 `YouTube-Chinese-Localizer-0.7.0.3-Standard-Offline-Setup.exe` 后直接双击安装即可。Standard 已是单文件安装包，
...
模型不会下载或占用磁盘。若电脑暂时不联网，可不勾选模型，之后从兼容模型 Release 下载安装独立模型包。四段版本号的最后一段是程序迭代号，例如 `0.7.0.3` 继续使用 `0.7.0` 模型包。
...
失败，仍可直接开始尝试。最多两个下载/预处理任务可并行，Whisper、
...
任务开始后，每一行都会独立显示当前阶段和进度。处理前可选中一行并点击“移除所选”；项目创建后可点击
“打开项目”直接查看这一项的 `source`、`subtitles`、`rendered` 和 `logs`。运行期间输入区会暂时锁定，
防止误粘贴或修改导致任务状态错位。
...
开发版桌面端在队列失败或点击“停止任务”后，会启用“重试未完成任务”。它只重新提交失败或未启动的项目，
