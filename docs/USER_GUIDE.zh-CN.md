...
下载 `YouTube-Chinese-Localizer-0.7.0.4-Standard-Offline-Setup.exe` 后直接双击安装即可。Standard 已是单文件安装包，
...
模型不会下载或占用磁盘。若电脑暂时不联网，可不勾选模型，之后从兼容模型 Release 下载安装独立模型包。四段版本号的最后一段是程序迭代号，例如 `0.7.0.4` 继续使用 `0.7.0` 模型包。
