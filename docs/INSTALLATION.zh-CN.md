...
3. 双击 Standard 时，在“选择本地模型”页按需勾选 Whisper Small、Whisper Medium 或 Local AI；安装器会从前三段兼容的模型 Release 下载选择的组件并校验 SHA-256。例如程序 `0.7.0.5` 使用 `0.7.0` 模型包。离线安装或使用 Complete 时，继续下一步手动安装模型包。
