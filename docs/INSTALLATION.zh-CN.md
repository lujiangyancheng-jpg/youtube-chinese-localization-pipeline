...
3. 双击 Standard 时，在“选择本地模型”页按需勾选 Whisper Small、Whisper Medium 或 Local AI；全新安装默认选择 Small，已有 Whisper 的升级不会重复选择。安装器会从前三段兼容的模型 Release 下载所选组件并校验 SHA-256，例如程序 `0.7.0.12` 使用 `0.7.0` 模型包。主动取消全部 Whisper 时会确认仅安装基础下载功能。
